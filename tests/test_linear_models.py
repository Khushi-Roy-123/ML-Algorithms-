...
def test_linear_regression_normal_equation_fit_predict():
    ...
    m = LinearRegression(solver="normal")
    m.fit(X, y)
    p = m.predict(np.array([[5.0], [6.0]]))

    assert np.allclose(m.coef_, [2.0], atol=1e-8)
    assert np.isclose(m.intercept_, 1.0, atol=1e-8)
    assert np.allclose(p, [11.0, 13.0], atol=1e-8)
...
def test_linear_regression_gradient_descent_fit():
    ...
    m = LinearRegression(solver="gd", lr=0.05, epochs=5000)
    m.fit(X, y)

    assert np.allclose(m.coef_, [2.0], atol=1e-2)
    assert np.isclose(m.intercept_, 1.0, atol=1e-2)
...
def test_logistic_regression_binary_prediction_shape():
    ...
    m = LogisticRegression(lr=0.1, epochs=2000)
    m.fit(X, y)
    p = m.predict(X)
...
def test_softmax_regression_multiclass_fit_predict():
    ...
    m = SoftmaxRegression(lr=0.2, epochs=2500)
    m.fit(X, y)
    p = m.predict(X)

    assert p.shape == y.shape
    assert set(np.unique(p)).issubset({0, 1, 2})
    assert np.mean(p == y) >= 0.95
