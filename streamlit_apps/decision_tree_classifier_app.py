...
class A:
    def __init__(self):
        self.t = "Decision Tree Classifier"

    def d(self, n):
        x, y = make_blobs(n_samples=n, n_features=2, centers=3, cluster_std=1.35, random_state=22)
        return x, y
    ...
    def r(self):
        st.title(self.t)
        n = st.sidebar.slider("n", 90, 700, 280)
        md = st.sidebar.slider("md", 1, 12, 4)
        ms = st.sidebar.slider("ms", 2, 20, 2)
        x, y = self.d(n)
        xt, xv, yt, yv = train_test_split(x, y, test_size=0.25, random_state=42)
        m = DecisionTreeClassifier(max_depth=md, min_samples_split=ms)
        m.fit(xt, yt)
        ypv = m.predict(xv)
