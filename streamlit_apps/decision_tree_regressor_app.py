...
class A:
    def __init__(self):
        self.t = "Decision Tree Regressor"

    def d(self, n, s):
        x, y = make_regression(n_samples=n, n_features=1, noise=s, random_state=12)
        return x, y
    ...
    def r(self):
        ...
        m = DecisionTreeRegressor(max_depth=md, min_samples_split=ms)
        m.fit(xt, yt)
        yp = m.predict(x)
        ypv = m.predict(xv)
