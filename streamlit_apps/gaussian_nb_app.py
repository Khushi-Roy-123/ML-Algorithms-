...
class A:
    ...
    def d(self, n):
        x, y = make_blobs(n_samples=n, n_features=2, centers=3, cluster_std=1.25, random_state=10)
        return x, y
    ...
    def r(self):
        st.title(self.t)
        n = st.sidebar.slider("n", 90, 600, 260)
        v = st.sidebar.slider("var", 1e-12, 1e-2, 1e-9, format="%.12f")
        x, y = self.d(n)
        xt, xv, yt, yv = train_test_split(x, y, test_size=0.25, random_state=42)
        m = GaussianNB(var_smoothing=v)
        m.fit(xt, yt)
        ypv = m.predict(xv)
